...
function Navbar({ isDarkTheme, setIsDarkTheme }) {
	...
	useEffect(() => {
		if (isHoveringLogo) {
			document.getElementById("logo-up").style.transform =
				"translate(-0.25ch, -0.5ch)";
			document.getElementById("logo-down").style.transform =
				"translate(0.25ch, 0.5ch)";
		} else {
		...
		>
			<nav className="navbar">
				...
				<Link to={"/"}>
					...
					>
						<h2 id="logo-up">{"<"}Punit&nbsp;</h2>
						<h2 id="logo-down">{"Tripathi/>"}</h2>
					</motion.div>
				</Link>
				...
				<ul className="navbar-links">
					...
					<li>
						<Link to="/" className="navbar-text-link">
							Home
						</Link>
					</li>
					<li>
						<Link to="/projects/" className="navbar-text-link">
							Projects
						</Link>
					</li>
					<li>
						<a
							...
							className="navbar-text-link resume"
							target="_blank"
						>
							Resume
						</a>
					...
					<div
						className={
							"dropdown-container dropdown-" + (isDarkTheme ? "dark" : "light")
						}
